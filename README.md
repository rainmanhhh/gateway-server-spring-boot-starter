...
- 权限校验流程说明
  ...
  3. 公共规则组应设置一条优先级最低的兜底规则：pattern=/**，type=roles，param=admin。其中admin为系统内置的超级管理员角色名，此规则的意义是，当所有规则其他规则都匹配不上，则只允许超级管理员访问当前uri。若不配置此兜底规则，那么当用户访问一个未被其他规则覆盖的uri时，会直接校验通过，有安全隐患
#### 通过命令行访问管理接口
例子：重载权限规则的bash脚本
```bash
curl -X GET \
